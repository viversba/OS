...
void mostrar(int sockEntrada, char perri[32])
{
    ...
    do
    {
		...
		if (m != 1)
		{
			send(sockEntrada, "No existe registro con el nombre ingresado", 200, 0);
			break;
		}
    } while (m == 0);
...
void *Servidor(void *arg)
{
    ...
    while (1)
    {
		...
		else if (o == '2')
		{
			...
			fclose(ap);
			char sizeR[10];
			sprintf(sizeR,"%d",size);
			send(sockEntrada, sizeR, 10, 0);
			do
			{
				char idmas[12] = "";
				int i = recv(sockEntrada, idmas, 12, 0);
				printf("en-%s",idmas);
				sscanf(idmas, "%d", &IdSa);
				printf("\nid-%d\n",IdSa);
		...
		else if (o == '3')
		{
			...
			send(sockEntrada, sizeD, 10, 0);

			char idmas[12];
			recv(sockEntrada, idmas, 12, 0);
			sscanf(idmas, "%d", &IdSa);
			printf("i %d \n",IdSa);
			fflush(stdout);
			borrar(sockEntrada, IdSa);
			char output[120];
			ap = fopen("serverDogs.log", "a+");
			if (ap == NULL)
			{
...
int main()
{
	...
	int NumCli = 0;
	semaforo = sem_open("sem",O_CREAT,0700,1);
	r = pipe(fd);
	r = write(fd[1],"able",4);
	if(r<0){
		perror("Couldn't create pipe in main");
		exit(-1);
	}
    while (1){
		if(NumCli<33){
